...
from meuSite.models import Servico
...
class Carrinho(object):
    ...
    def get_servico(self):
        ...
        for item in self.carrinho.values():
            servico = Servico.objects.get(id=item['id'])
            lista.append({'servico': servico,
                          'preco': item['preco'],
    ...
    def adicionar(self, servico_id, quantidade):
        ...
        if servico_id not in self.carrinho:
            self.carrinho[servico_id] = {'id': servico_id, 'preco': str(servico.preco), 'quantidade': quantidade}
        else:
            # Como o objeto sessão não é modificado, isto é, apenas o objeto carrinho foi alterado,
            # logo esta modificação não será salva!
            self.carrinho[servico_id]['quantidade'] += quantidade
